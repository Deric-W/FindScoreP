...
\begin{document}
    ...
    um ein gro{\ss}es Projekt zu simulieren.
    ...
    Dabei existieren zwei Varianten des Beispielprojekts, welche sich durch die Verwendung der
    ...
    \begin{center}
        \begin{tikzpicture}
            \begin{axis}[
                title=Zeit zum analysieren des Beispielprojekts,
                xlabel=Anzahl der standalone Targets,
                ylabel=Zeit in Mikrosekunden,
                width=0.75\textwidth,
                legend pos=north west
            ]
                \addplot table [x=targets, y=runtime, col sep=comma] {BenchmarkProject/runtime_auto.csv};
                \addlegendentry{mit AUTO}
                \addplot table [x=targets, y=runtime, col sep=comma] {BenchmarkProject/runtime_noauto.csv};
                \addlegendentry{ohne AUTO}
            \end{axis}
        \end{tikzpicture}
    \end{center}

    Es ist ersichtlich dass die Laufzeit superlinear wächst während das Verzichten auf
